...
export const FormChoixImage = ({
  ...
  const onClickBtn = () => {
    ...
    scrollToForm(); // Appeler la fonction pour faire défiler vers l'élément avec l'ID "form"
  };
  ...
  const handleClick = (answer) => {
    ...
    setTimeout(() => {
      ...
      // => Faire un if pour passer à la question 2 seulement si la réponse à la question 1 est "Hôtel Restaurante Panadería" sinon passer à la question 4
      if (idQuestion === 1) {
        if (answer.label === "Hôtel Restaurante Panadería") {
          setIdQuestion((prev) => prev + 1);
        } else {
  ...
  return (
    <div className="flex flex-col items-center">
      {/* La question */}
      <div id="form" className="py-2 mt-4 mb-6 lg:py-0 lg:mb-4">
      ...
      {/* Les reponses */}
      <div className="grid grid-cols-2 w-[85%] lg:w-[95%] lg:grid-cols-4 gap-4 pb-10 lg:pb-10 lg:mx-2 ">
        {oneData.reponses.map((answer, index) => (
          <button
            ...
            className={` ${styleAnswer} flex items-center justify-between   ${isDisabled
              ? " opacity-70 cursor-not-allowed "
              : ""
              }`}
            disabled={isDisabled}
          >
            <div className="flex flex-col text-black leading-none font-extrabold h-40 lg:h-40 lg:text-[16px] gap-2 lg:gap-0 lg:flex-col justify-center items-center px-2 lg:px-6 mx-auto py-3 lg:py-8 hover:text-white">
              {answer?.image && (
                <img
      ...
      <div className="flex justify-center">
        <button
          className="btn bg-[#000072] border-none rounded-[var(--rounded-button)] py-0 w-[90%] lg:w-[100%] h-14 font-bold text-white text-base lg:text-xl hover:bg-[#C60278]"
          onClick={onClickBtn}
        >
