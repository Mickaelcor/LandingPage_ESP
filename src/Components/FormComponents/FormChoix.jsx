...
export const FormChoix = ({
    ...
    const handleClick = (answer) => {
        ...
        setTimeout(() => {
            ...
            // => Faire un if si question 2 et réponse non => question 4

            if (idQuestion === 2 && answer.label === "NO") {
                setIdQuestion((prev) => prev + 2);
            } else {
                setIdQuestion((prev) => prev + 1);
            }
...
hover:border-[var(--border-answer-hover)] 
  hover:text-[var(--color-answer-hover)]`;
    ...
    return (
        <div className="flex flex-col items-center">
            {/* La question */}
            <div id="form" className="py-2 mt-4 mb-6">
            ...
            {/* Les reponses */}
            <div className="grid grid-row-6 w-[90%] lg:w-[65%] lg:grid-cols-2 gap-4 pb-10 lg:pb-10 lg:mx-2">
                {oneData.reponses.map((answer, index) => (
                    <button
                        ...
                        className={` ${styleAnswer} flex items-center justify-between shadow border rounded border-[#4689C2] lg:border-neutral-300 bg-white hover:bg-[#1C316B]  ${isDisabled
                            ? " opacity-70 cursor-not-allowed "
                            : ""
                            }`}
                        disabled={isDisabled}
                    >
                        <div className="flex w-full text-black leading-none font-extrabold h-15 lg:h-48 lg:text-[16px] gap-2 lg:gap-0 lg:flex-col justify-center items-center px-2 lg:px-6 mx-auto py-3 lg:py-8 hover:text-white">
                            {answer?.image && (
                                <img
